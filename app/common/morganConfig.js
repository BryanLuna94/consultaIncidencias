    ...
    function streamWriteCustom(str) {
        try {
            ...
            if (datos) {
                ...
                let indexSeparador = paramsFormat.indexOf('/:');
                let omitirUrl = paramsFormat.substring(0, indexSeparador);
                let parametersFormatArray = paramsFormat.substring(indexSeparador).split('/:');
                let parametersValues = url.substring(endpoint.length + omitirUrl.length, url.length);
                ...
                let level = 'INFO';
                if (!statusCodeOK.includes(status)) {
                    level = 'ERROR';
                }

                if (level === 'ERROR' || config.elasticLogging.logAllResponse) {
